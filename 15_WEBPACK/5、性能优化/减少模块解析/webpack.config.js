module.exports = {
  ...
  module:{
    ...
    //因为不解析的本质也就是不进行单纯的分析和替换依赖名称的操作(即模块解析操作的本质为AST语法树分析的过程，同时反映该过程中实际做的事情也就是将导入的语句进行替换--注意没有导出语句)，
    //但是如果自身被其它文件依赖了，那么自身还是会被当作一个模块进行处理(也就是在资源列表中是存在的，只是不存在编译流程中的依赖列表中了(本身起始也没有依赖，此时更是不进行分析和记录依赖了)，也就能够在编译结果中访问到了)
