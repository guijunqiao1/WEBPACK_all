...
var MinCssExtractPlugin = require("min-css-extract-plugin");

var HtmlWebpackPlugin = require("html-webpack-plugin");

module.exports = {
  mode: "production",
  //需要注意的是一般分包用于生产环境，因为开发环境的传输性能已经可以被热替换优化完毕了
  entry: {
    page1: "./src/page1",
    page2: "./src/page2"
  },
  ...
  Optimization: {
    splitChunks: {
      //分包策略
      chunks: all,
      //针对所有的chunk都要应用分包策略，默认值为async表示的是仅针对异步chunk应用分包策略--webpack的懒加载中提到异步chunk
      maxSize: 60000,
      //控制分出来的"公共"chunk的最大字节数,若超过则对该公共chunk进行分包
      automaticNameDelimiter: ".",
      //当前配置控制的是终端控制台打印出来的资源列表中公共分包的资源名称中的分隔符
      minChunk: 2,
      //控制的是最小的chunk的引用数，当多少个"chunk"引用公共模块的时候将公共模块分出一个chunk包出来,默认值为1
      minSize: 30000,
      //当进入了minChunk的分包前提的模块满足当前minSize的配置要求的时候才进行实际的分包--也就是minChunk为公共模块分包的首要条件，而当前minSize为次要条件
      // 该配置项的默认值为30000byte
      ...
      cacheGroups: {
        ...
        styles: {
          ...
          minSize: 0,
          test: /\.css$/,
          minChunks: 2 //需要注意的是没有显式配置priority则默认值为0

        }
      }
    }
  },
  plugins: [new CleanWebpackPlugin(), new MinCssExtractPlugin({
