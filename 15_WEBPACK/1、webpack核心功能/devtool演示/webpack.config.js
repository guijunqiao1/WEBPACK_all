...
module.exports = {
  mode: "development",
  entry: {//打包的入口文件配置,在开发者模式下为require("./main.js")的形式
  ...
  output: {//打包的出口文件配置，打包返回的文件的配置项
    ...
    filename:"script/[name].[chunkhash:5].js",//[chunkhash]用于表示当前的chunk单独的hash值，并且当前的chunk中的内容发生改变并不会导致所有的assets的hash值发生改变同时不同的chunk之间的hash不同，相同的chunk的不同的内容的hash也不同--这种情况可以合理提示浏览器使用缓存机制
  ...
  watch:true,//固定开启打包监控--和nodemon同理
  module:{//配置模块--主要配置的是模块解析(loader)的时候的规则
    rules:[
      {
        test:/index\.js$/,//正则表达式,匹配模块的路径,其中\.为转义为.的含义，$表示匹配的字符串结束
        use:[
          {
